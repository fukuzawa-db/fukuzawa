...
ScanFromJI_OneHash::ScanFromJI_OneHash(Operator *innerOpeNode_, Operator *outerOpeNode_){
...
ScanFromJI_OneHash::ScanFromJI_OneHash(Operator *innerOpeNode_, Operator *outerOpeNode_, JOIN_OPERATION_NODE *joinOpeNode_){
	innerOpeNode = innerOpeNode_;
	outerOpeNode = outerOpeNode_;
...
ScanFromJI_OneHash::~ScanFromJI_OneHash(){
}

void ScanFromJI_OneHash::delHash(){
	innerHash->db->close();
	innerHash->db->remove();
	delete innerHash->db;
	delete innerHash;
...
void ScanFromJI_OneHash::init(DbEnv *env_){
...
int ScanFromJI_OneHash::getRow(PosVal **pv_, unsigned int pvNum_){
	int returnCode;
	...
	if(getRowFlag == 0){
		...
		if(returnCode != 0){
			...
			for(unsigned int i = 0; i < nodeNum; i++)
				hashkey += hashFunc(pv[outerPos[0]]->position);
	...
	while(1){
		...
		if(returnCode == 0){
			...
			for(posIndex = 0; posIndex < nodeNum; posIndex++){
				if(!(pv[innerPos[posIndex]]->position == pv[outerPos[posIndex]]->position))
					break;
			}
			if(posIndex == nodeNum){
...
unsigned int ScanFromJI_OneHash::getAttriNum(void){
	return attriNum;
}

void ScanFromJI_OneHash::getAttriRec(ATTRIBUTE_REC **attriRec_, unsigned int attriNum_){
...
void ScanFromJI_OneHash::initHash(void){
	
	innerHash = new ONESIDE_HASH_NODE;
	tmpnam(tmpFileName);
	innerHash->dbName = tmpFileName;
	innerHash->db = new BDBOpe(env, innerHash->dbName, false);
	innerHash->db->open();
	
	innerHash->ScanNode = new ScanHash(innerHash->db, attriRec, innerAttriNum+1);
}

void ScanFromJI_OneHash::storeInnerNode(void){
	unsigned int hashkey;
	int returnCode;
	...
	while(1){
		...
		for(unsigned int i = 0; i < nodeNum; i++)
			hashkey += hashFunc(pv[innerPos[0]]->position);
		hashkey %= HASH_SIZE;
...
void ScanFromJI_OneHash::setJoinPos(void){
...
int ScanFromJI_OneHash::getInnerDataBuffSize(void){
	int size = 0;
...
unsigned int ScanFromJI_OneHash::getDataBuffer(char *dataBuffer_, unsigned int dataBufSize_, unsigned int startPos_, unsigned int getNum_)
{
	unsigned int size = 0;
